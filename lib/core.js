...
function getData(variable, report, path, line) {
  const name = variable[2].replace(/(\[|\])/g, '');
    ...
    for (let line = liner.next(); line; line = liner.next()) {
      ...
      try {
        ...
        if (/^(import\s{1}(.*?)$)/.test(line)) {
          ...
          if (ext) {
            const importPath = PATH.join(baseDir, name);
            switch (ext) {
              case '.code':
        ...
        if (Array.isArray(block)) {
          ...
          switch (d) {
            ...
            case 3: {
              block = block.input.match(/(.*?)\s{1}(.*?)\s{1}([^{]+)/i);
              if (
                /^\s*([0-9a-zA-Z_]+)\s{1}([0-9]+)\s{1}(.*?)+$/.test(block.input) === false
              )
                return addError(
                  errors,
            ...
            case 4: {
              if (
                /^(OPT|REQ|RES|PROC|MARK)\s{1}\{/.test(
                  block.input,
                ) === false
              )
                return addError(
        ...
        switch (d) {
          ...
          case 3: {
            if (!Array.isArray(variable)) break;
            if (
              /^(COMP|GET|PUT|POST|PATCH|DELETE)\s{1}[^{](.*?)$/.test(variable.input) === false
            )
              return addError(
                errors,
          ...
          default: {
            ...
            switch (b?.NAME) {
              ...
              case 'RES': {
                if (d > 4) {
                  if (
                    ...
                    /^\s*[0-9a-zA-Z\uAC00-\uD7A3-_!=><]+$/.test(variable[1]) === false
                  )
                    return addError(
                      errors,
              ...
              case 'OPT': {
                if (
                  !Array.isArray(variable) ||
                  /^\s*([0-9a-zA-Z_]+)\s{1}(true|false)$/.test(variable.input) === false
                )
                  return addError(
                    errors,
...
function compile(projectDir) {
  ...
  try {
    for (let entry of FS.readdirSync(apiDir)) {
      const entryPath = PATH.join(apiDir, entry);
      if (FS.lstatSync(entryPath).isDirectory()) continue;
      if (entry.startsWith('._')) continue;
      if (PATH.extname(entry) !== '.api') continue;
  ...
  const result = getManufacture(paths, projectDir, errors);
...
function generate(projectDir) {
  const compileResult = compile(projectDir);
...
function parseApi(content, options) {
  ...
  const tmpFile = PATH.join(tmpDir, 'input.api');

  try {
    FS.writeFileSync(tmpFile, content, 'utf8');
    const result = getManufacture([tmpFile], baseDir, errors);
...
function parseCode(content) {
  ...
  const result = getCodeFromString(content, data, report, errors);
...
function parseStruct(content) {
  const errors = [];
  const data = {};
  const report = { def: {}, ref: {} };

  const result = getStructFromString(content, data, report, errors);
