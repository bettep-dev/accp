...
/**
 * Flush buffer to file synchronously.
 */
...
/**
 * Print error message: ✗ msg (red) + optional → detail lines (dim)
 * @param { string } msg
 * @param { array } details
 */
function logError(msg, details = []) {
  ...
  for (const row of details) {
    if (Array.isArray(row)) {
      console.log(`    ${"→".dim} ${row.join(" :: ").dim}`);
    } else {
      console.log(`    ${"→".dim} ${String(row).dim}`);
    }
  }
...
/**
 * Print stat line: LABEL  value (label bold, value dim)
 * @param { string } label
 * @param { string } value
 */
function logStat(label, value) {
  console.log(`  ${label.bold.white}  ${value.dim}`);
}

/**
 * Legacy error log adapter — all callers pass "brightRed".
 * @param { string } content
 * @param { string } _color
 * @param { array } array
 */
function setLog(content, _color, array = []) {
  return logError(content, array);
}

/**
 * Generate output file recursively and collect stats.
 ...
 * @param { string } base - Base directory for relative path display.
 * @param { object } stats - { dirs, files } counter.
 */
function setFile(dir, loc, base, stats) {
  let entries = FS.readdirSync(loc, { withFileTypes: true });

  for (const entry of entries) {
    if (entry.name.startsWith("._")) continue;

    let src = PATH.join(loc, entry.name);
    let dst = PATH.join(dir, entry.name);
    ...
    if (entry.isDirectory()) {
      ...
      setFile(dst, src, base, stats);
    } else {
      FS.writeFileSync(dst, FS.readFileSync(src));

      logSuccess(PATH.relative(base, dst));
...
function setExcute() {
  ...
  try {
    for (let entry of FS.readdirSync(api, { withFileTypes: true })) {
      if (entry.isDirectory()) continue;

      if (entry.name.startsWith("._")) continue;

      if (PATH.extname(entry.name) !== ".api") continue;

      paths.push(PATH.join(api, entry.name));
    }
  } catch (_error) {
    logError("Could not find api file");

    return false;
  }

  if (paths.length < 1) {
    logError("Could not find file with api extension");

    return false;
  }

  for (const p of paths) {
    logSuccess(`Parsed ${PATH.basename(p)}`);
  }

  let pattern = PATH.join(process.cwd(), "pattern");

  try {
    for (let entry of FS.readdirSync(pattern, { withFileTypes: true })) {
      if (entry.name.startsWith("._")) continue;

      if (PATH.extname(entry.name) !== ".js") continue;

      patterns.push(PATH.join(pattern, entry.name));
    }
  } catch (_error) {
    logError("Could not find pattern file");
  ...
  logSuccess(`Found ${patterns.length} patterns`);

  console.log();

  for (const path of paths) {
    const manufacture = getManufacture([path]);
    ...
    if (manufacture) {
      for (let pattern of patterns) {
        try {
          ...
          logSuccess(`${PATH.basename(pattern)}   generated`);
          console.log(`    ${"→".dim} ${pattern.dim}`);
        } catch (error) {
          if (error.message.indexOf("require(...)") > -1) {
            logError("Include the following code in your pattern file.", [
              pattern,
            ]);

            console.log(
              `  ${"module.exports = function( OBJ, GEN ) { /* Please write your code */ }".dim}\n`,
            );

            return false;
          }

          logError(error.message, [pattern]);
        }
      }
    }
  }
...
function setExamples() {
  const loc = PATH.join(__dirname, CONFIG.EXAMPLES);
  const dir = PATH.join(process.cwd(), COMMAND.examples);

  if (FS.existsSync(dir)) {
    logError(`The "${COMMAND.examples}" folder already exists.`);

    return false;
  }

  console.log(`  ${"◌".yellow} Scaffolding ${COMMAND.examples.bold}...\n`);

  FS.mkdirSync(dir);

  const stats = { dirs: 0, files: 0 };

  setFile(dir, loc, dir, stats);

  console.log();

  logStat("FILES ", `${stats.files} files · ${stats.dirs} dirs`);
}
...
function getCode(path, data, report) {
  ...
  let l = 0;
  let n = null;
  const p = PATH.join(process.cwd(), path);

  if (FS.existsSync(p) === false)
    return setLog("Code file not found.", "brightRed", [p]);

  /* Read file line by line. */
  const liner = new LINE(p);

  for (let line = liner.next(); line; line = liner.next()) {
    l++;
    ...
    if (Array.isArray(block)) {
      ...
      switch (d) {
        ...
        case 2: {
          ...
          if (/^[0-9a-zA-Z_]+$/.test(block[1]) === false)
            return setLog(
              'Code names must have the structure "([0-9a-zA-Z_]) (.*?) {}".',
              "brightRed",
              [
        ...
        case 3: {
          ...
          if (
            !Array.isArray(block) ||
            /^\s*([0-9]+)\s{1}(.*?)$/.test(block.input) === false
          )
            return setLog(
    ...
    const variable = line.match(/(.*?)\s{1}(.*?)$/i);

    if (Array.isArray(variable)) {
      switch (d) {
        case 3: {
          if (
            /^\s*([0-9a-zA-Z_]+)\s{1}(.*?)$/.test(
              variable.input,
            ) === false
          )
            return setLog(
...
function getStruct(path, data, report) {
  ...
  for (let line = liner.next(); line; line = liner.next()) {
    ...
    try {
      ...
      const variable = /([^\s]+)\s{1}([^\s]+)\s?(.*?)$/i.exec(line);
      ...
      switch (d) {
        case 2: {
          ...
          if (
            /^\s*([0-9a-zA-Z_]+)\s{1}([0-9a-zA-Z_[\]]+)\s{1}(.*?)+$/.test(variable.input) === false
          )
            return setLog(
        ...
        case 3: {
          ...
          if (
            /^[0-9a-zA-Z가-힣-_!=><]+$/.test(variable[1]) === false
          )
            return setLog(
              'Struct data must have the structure "([0-9a-zA-Z가-힣-_!=><]+) (.*?)".',
              "brightRed",
...
function getManufacture(paths) {
      ...
      try {
            ...
            case 2: {
              ...
              if (
                ...
                if (pop.PARAM && pop.PARAM.length > 0) {
                  const reqList = pop.REQ || [];

                  for (let param of pop.PARAM) {
                    const reqParam = reqList.find((r) => r.NAME === param);
                    ...
                    if (reqParam.ARRAY === true || !['int', 'string'].includes(reqParam.CLASS.toLowerCase()))
                      return setLog(
                        `Path parameter ":${param}" must be Int or String type.`,
                        "brightRed",
                        [[path, pop.LINE], pop.NAME],
                      );
                  }
        ...
        if (Array.isArray(block)) {
          ...
          switch (d) {
            ...
            case 4: {
              if (
                /^(OPT|REQ|RES|PROC|MARK)\s{1}\{/.test(block.input) ===
                false
              )
                return setLog(
        ...
        const variable = line.match(/([^\s]+)\s{1}([^\s]+)\s?(.*?)$/i);
        ...
        switch (d) {
          case 2: {
            ...
            if (
              /^(BASE)\s{1}[^{](.*?)$/.test(variable.input) === false
            )
              return setLog(
          ...
          case 3: {
            ...
            if (
              /^(COMP|GET|PUT|POST|PATCH|DELETE)\s{1}[^{](.*?)$/.test(
                variable.input,
              ) === false
            )
              return setLog(
            ...
            switch (variable[1]) {
              ...
              case "DELETE": {
                ...
                let paramMatch = paramRegex.exec(variable[2]);
          ...
          default: {
            ...
            switch (b?.NAME) {
              ...
              case "OPT": {
                if (
                  !Array.isArray(variable) ||
                  /^\s*([0-9a-zA-Z_]+)\s{1}(true|false)$/.test(
                    variable.input,
                  ) === false
                )
                  return setLog(
              ...
              case "PROC": {
                if (
                  !Array.isArray(variable) ||
                  /^\s*([0-9]+)\s{1}([0-9a-zA-Z_]+)\.([0-9a-zA-Z_]+)/.test(variable.input) === false
                )
                  return setLog(
  ...
  for (let [key, value] of Object.entries(report.struct.ref)) {
    if (key in report.struct.def) continue;
  ...
  const apiEndpoints = result.class.reduce((sum, r) => sum + r[1], 0);
  const apiClasses = result.class.length;
  const codeDefinitions = result.code.reduce((sum, r) => sum + r[1], 0);
  const codeGroups = result.code.length;
  const structCount = Object.keys(report.struct.def).length;

  logStat("API   ", `${apiEndpoints} endpoints · ${apiClasses} classes`);
  logStat("CODE  ", `${codeDefinitions} definitions · ${codeGroups} groups`);
  logStat("STRUCT", `${structCount} structures`);
...
if (!COMMAND.compile && !COMMAND.examples) {
  COMMAND.help();
}

function runCommand(fn, doneLabel) {
  console.log(`\n${"accp".bold} v${VERSION}\n`);
  ...
  if (result !== false) {
    console.log(`\n  ${"✓".green} ${doneLabel} in ${elapsed}ms\n`);
  }
}

if (COMMAND.compile) {
  runCommand(setExcute, "Compiled");
}

if (COMMAND.examples) {
  runCommand(setExamples, "Done");
}
